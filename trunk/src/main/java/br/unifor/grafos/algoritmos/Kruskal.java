...
public class Kruskal {
	...
	Kruskal() {
		...
		todasArestas = new TreeSet(new Aresta()); // Create empty priority queue
		novasArestas = new Vector(MAXIMO_NOS); // Create vector for MST edges
	}

	public int iniciar(int num_nodes, int vertices[], Integer matriz[][]) {
		...
		for (int i = 0; i < num_nodes; i++) {
			for (int j = 0; j < num_nodes; j++) {
				...
				destino = j;
				valor = matriz[i][j];
				todasArestas.add(new Aresta(origem, destino, valor)); // Update
																		// priority
																		// queue
				if (nos[origem] == null) {
					// Create set of connect components [singleton] for this
					// node
		...
		for (int i = 0; i < size; i++) {
			...
			if (todasArestas.remove(arestaAtual)) {
				...
				// This is a serious problem
				System.out
						.println("TreeSet should have contained this element!!");
				System.exit(1);
			}
	...
	/**
	 * Imprime as novas arestas.
	 */

	public void imprimeSolucaoParcial() {
		// imprime all new Edges
		System.out
				.println("**** Imprimindo Solu��o Parcial **** Conjuntos disjuntos da floresta \n");
		for (int i = 0; i < novasArestas.size(); i++) {
			System.out.println("Aresta : " + i + " com peso - "
					+ ((Aresta) novasArestas.get(i)).valor());
		}
		System.out
				.println("**** Imprimindo Solu��o Parcial **** Conjuntos disjuntos da floresta \n");
	}
	...
	public void imprimeFilaDePrioridade() {
		...
		int contador = 0;
		while (i.hasNext()) {
			System.out.println("Aresta : " + contador + " com peso - "
					+ ((Aresta) i.next()).valor());
			contador++;
		}
		System.out.println("**** Imprimindo Fila de Prioridade **** \n");
